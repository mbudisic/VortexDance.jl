...
begin
	xax = minimum(first.(sol_matrix)):0.25:maximum(first.(sol_matrix))
	yax = minimum(last.(sol_matrix)):0.25:maximum(last.(sol_matrix))
end;
...
Smoothing exponent = $(@bind smoothing_exp PlutoUI.Slider(-2:0.1:1, show_value=true))

Field = $(@bind fieldsel PlutoUI.Select([:vorticity, :stream]))
"""
...
md"""
The figure was generated by first evaluating the velocity field and stream function along the grid.
"""
...
# ╔═╡ 7f3903c9-e3a5-4a47-abe0-9e31b95856ce
Ψ = [
	VortexDance.biotsavart(Vec2D([xx,yy]), vortex_positions, Γ;
	core=10 .^ smoothing_exp,fieldtype=:stream) 
	for xx in xax,  yy in yax];

# ╔═╡ e17b1f5a-9cfb-46ba-90c3-752abde535d3
ω = [
	VortexDance.biotsavart(Vec2D([xx,yy]), vortex_positions, Γ;
...
md"""
And then overlaying the heatmap/arrow plots.
"""
...
velocity_figure = with_theme(theme_minimal()) do
	...
	if fieldsel == :vorticity
		F = ω
	else
		F = Ψ
	end
	
	h = Makie.heatmap!(ax,xax, yax,F,
		colormap=:balance,
	colorrange = [-1,1].*maximum(abs.(F)))
	Makie.arrows!(xax, yax,first.(u)/10, last.(u)/10; 
	normalize=false,
	color=(:black, 0.5),arrowsize=5)
