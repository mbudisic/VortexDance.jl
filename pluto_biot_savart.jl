...
# ╔═╡ b791cea7-04e7-49d4-acf3-6c6d41aab5cb
using DifferentialEquations

# ╔═╡ b5e8b09e-50fd-4aea-983b-3016fe256f00
using Plots

# ╔═╡ a8088392-19fb-4de0-95a4-9dd360a195ad
using CairoMakie
...
corebind = @bind corexp PlutoUI.Slider(-9:0)
...
# ╔═╡ 2d7c78ec-1453-4c86-accf-b049bce94871
biotsavart(ps,ps,Γ;core=coreval)

# ╔═╡ 7e2b4442-9deb-42d4-b4ba-7ac63395ed5e
function vortexdance( p_initial, Γ, T )

    velocityfield(u, p, t) = first.(biotsavart(u, u, p))
...
# ╔═╡ 5693990f-69b7-47ae-9d3a-6062fac4e4e7
velocityfield(u, p, t) = first.(biotsavart(u, u, p))


# ╔═╡ 1187589b-0fbf-461f-b1e5-8bb38e6f1863
a = velocityfield(ps, Γ, 0.1)

# ╔═╡ 645bf486-63ef-49c6-8ba6-a7638a847aa8
typeof(a)

# ╔═╡ fafbbf89-f491-43ba-b26f-cfd45f72b1b5


# ╔═╡ 64af78b8-e1cf-4811-989e-bc2378a2e570
Nslider = @bind N PlutoUI.Slider(3:7,show_value=true)
...
begin
	ps1 = [Point(c) for c in eachcol(rand(2,N) * 3)]
	Γ1 = randn(N) + 2 .* sign.(randn(N))
	Γ1
end

# ╔═╡ 2efb0096-168d-4dfb-9408-b4f56f3920b8
sol,diffeq = vortexdance( ps1, Γ1, 10. )

# ╔═╡ 8cfebcca-ffad-47c7-80da-3d305a690808
begin
	t = 0:0.01:10
	sols = sol(t)
end
...
begin
	...
	ax = Axis(fig[1,1], xlabel = "x", ylabel = "y", title = "Plot of $N vortices")

	Γrange = Tuple([-1,1] * maximum(abs.(Γ1)))
	
	for k = 1:N
		x = first.([snapshot[k] for snapshot in sols])
		y = last.([snapshot[k] for snapshot in sols])
		Makie.lines!(ax,x,y,
			legend_label = "Vortex $k",
			color=ones(length(x),)*Γ1[k], 
			colormap=:balance,
			colorrange=Γrange
		)
	end
	Colorbar(fig[1,2],limits=Γrange,colormap=:balance)
